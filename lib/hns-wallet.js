...
class HNSWallet {
  ...
  encryptSeed(passphrase) {
    const pbkey = this.crypto.pbkdf2.derive(this.crypto.sha256, passphrase, SALT, ITERATIONS, 32);
    const raw = Buffer.from(this.phrase, 'ascii');
    const iv = this.crypto.random.randomBytes(16);
    const cipher = this.crypto.aes.encipher(raw, pbkey, iv);
    return {iv, cipher};
  }
  ...
  static fromHNSResourceJSON(resource, passphrase, main, backend) {
    for (const rec of resource.records) {
      ...
      const wallet = this.fromEncryptedPhrase(obj, passphrase, main, backend);
  ...
  static fromEncryptedPhrase(obj, passphrase, main = false, backend) {
    const pbkey = backend.pbkdf2.derive(backend.sha256, passphrase, SALT, ITERATIONS, 32);
    const {iv, cipher} = obj;
    const phrase = backend.aes.decipher(cipher, pbkey, iv);
